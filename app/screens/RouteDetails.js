...
function Card(props) {
  ...
  pickImage = () => {
    props.pickImage(props.pin["_id"]);
  }

  return (
    <TouchableWithoutFeedback onPress={props.onPress}>
      <View style={mapStyles.card}>
        <Image source={props.pickedImage[props.pin["_id"]] ? {uri: props.pickedImage[props.pin["_id"]]} : source} style={mapStyles.cardImage} resizeMode='cover'/>
        ...
        <View style={mapStyles.addPhoto}>
          <Icon name='add' size={18} color={'#ffffff'} onPress={pickImage}/>
        </View>
...
class RouteDetailsScreen extends Component {
  ...
  _pickImage = async (placeid) => {
    ...
    console.log(result);
    let images = this.state.pickedImage;
    images[placeid] = result.uri
    this.setState({
      pickedImage: images
    });
  }
